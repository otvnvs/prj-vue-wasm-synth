...
const startCableDrag = (nodeId, event) => {
  ...
  event.stopPropagation(); // don't trigger module drag
  updateJackPositions();
...
// ─── MODULE DRAG (reordering) ────────────────────────────────────────────────
// We keep an ordered list of node IDs and allow drag-to-reorder via the header.
const nodeOrder = ref([]); // array of node IDs in display order
const draggingModuleId = ref(null);
const dragOverModuleId = ref(null);

// Keep nodeOrder in sync when nodes are added/removed
const syncNodeOrder = () => {
  const current = Object.keys(activeNodes.value);
  // add new nodes at end, remove deleted ones
  nodeOrder.value = [
...
// Computed-like: ordered list of [id, node] pairs
const orderedNodes = () => {
  syncNodeOrder();
  return nodeOrder.value.map(id => [id, activeNodes.value[id]]).filter(([,n]) => n);
};
...
<template>
  <main class="patchbay-app">
    ...
    <section class="panel activation">
      ...
      <div v-else class="status-success">✓ Rack Live &nbsp;·&nbsp; Drag <span class="hint-green">OUT</span> → <span class="hint-blue">IN</span> to patch &nbsp;·&nbsp; Double-click wire to cut &nbsp;·&nbsp; Drag header to reorder</div>
    </section>
    ...
    <!-- ── SVG CABLE CANVAS ─────────────────────────────────────────────── -->
    <svg ref="svgEl" class="patch-cable-canvas" v-if="isReady" xmlns="http://www.w3.org/2000/svg">
    ...
    <section class="rack-enclosure" v-if="Object.keys(activeNodes).length > 0">
      <div class="rack-grid">
        ...
        <!-- Master output (fixed, always first) -->
        <div class="rack-chassis-module master-output-chassis">
          ...
          <div class="jack-rack-row">
            <div class="jack in-jack" data-node-id="main_output"
              @mouseup.stop="dropCableOnInput('main_output', $event)"
        ...
        <!-- Draggable node modules -->
        <div
          v-for="[id, node] in orderedNodes()"
          :key="id"
          class="rack-chassis-module"
          :class="{
            'is-dragging': draggingModuleId === id,
            'drag-over': dragOverModuleId === id
          }"
          @dragover.prevent
        ...
        >
          <div
            class="chassis-header draggable-header"
            @mousedown="startModuleDrag(id, $event)"
            @mousemove="draggingModuleId && draggingModuleId !== id && onModuleDragEnter(id)"
          >
            <div class="header-left">
          ...
          <div class="jack-rack-row">
            <div v-if="node.inputs > 0" class="jack in-jack" :data-node-id="id"
              @mouseup.stop="dropCableOnInput(id, $event)"
              :class="{ 'pulse-highlight': isDraggingCable && dragSourceId !== id }">
              <div class="jack-socket"></div>
              <span class="jack-label">SIGNAL IN</span>
            </div>
            <div v-if="node.outputs > 0" class="jack out-jack" :data-node-id="id"
          ...
          <div class="sliders-grid" v-if="node.parameters && node.parameters.length">
            <div v-for="param in node.parameters" :key="param.id" class="slider-box">
              ...
              <div v-else class="control-wrapper">
                ...
                <input type="range" :min="param.min" :max="param.max" step="0.01"
                  :value="param.value" @input="updateParam(id, param.id, $event.target.value)" />
              </div>
            </div>
          </div>
        </div>
...
h1 { display: none; } /* title removed for clean look */
