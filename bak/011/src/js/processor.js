...
class WasmGraphProcessor extends AudioWorkletProcessor {
    constructor() {
        ...
        this.port.onmessage = async (event) => {
            ...
            if (payload.type === 'UNPATCH_CABLE') {
                this.connections = this.connections.filter(
                    edge => !(edge.sourceId === payload.sourceId && edge.targetId === payload.targetId)
                );
                this.recompileGraphTopology();
            }
...
// Inside src/js/processor.js
    ...
    process(inputs, outputs) {
        // FIXED EXTRUCTION: Extract the absolute raw Float32Array channel buffer view
        // outputs[0] selects the first output device slot; [0] selects the Left/Mono channel
        const outputChannel = outputs?.[0]?.[0];
        if (!outputChannel) return true; // Keep thread alive if audio graph isn't loaded yet

        // Clear the master accumulation bus first
        this.buses['main_output'].fill(0.0);

        // Step sequentially through our topologically sorted nodes list
        for (const nodeId of this.executionOrder) {
            ...
            // Skip isolated, bypassed modules to keep processing loops light
            if (!hasInputs && !hasOutputs && node.type !== 'generator') {
                continue;
            }

            // 1. Gather audio input frames by checking connected parent buses
            ...
            // 2. Invoke the compiled C plugin execution loop function pointer
            ...
            // 3. Mirror the output data frames onto our shared tracking bus map layer
        ...
        // Pass the completed mixed signal stream directly out to the hardware speakers
